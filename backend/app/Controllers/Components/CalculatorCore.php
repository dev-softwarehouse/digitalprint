...
class CalculatorCore extends Component
{
    public $useModels = array(
        ...
        'PrintShopConfigRealizationTime',
        'PrintShopRealizationTime',
    ...
    public function __construct()
    {
        ...
        $this->AuthAssistant = AuthAssistant::getInstance();
    }
    ...
    public function calculate($groupID, $typeID, $amount, $volume, $products, $tax)
    {
        ...
        foreach ($products as $product) {
            ...
            $formatInfo = $this->CalculateStorage->getFormat($product['formatID']);
            $this->formatSlope = $formatInfo['slope'];
        ...
        if (!isset($this->realisationTimes[$typeID])) {
            ...
            $realisationTimes = $this->PrintShopRealizationTime->getRealizationTimes(1);
            ...
            if ($realisationTimes && $realisationTimesDetails) {
                ...
                foreach ($realisationTimes as $rtKey => $rt) {
                    ...
                    if (!isset($realisationTimesDetails[$rt['ID']]) || !$realisationTimesDetails[$rt['ID']]) {
                        if ($rt['active'] == 0) {
                    ...
                    if (isset($realisationTimesDetails[$rt['ID']])) {
                        foreach ($realisationTimesDetails[$rt['ID']] as $row) {
                            ...
                            if ($row['typeID'] > 0 && $realisationTimes[$rtKey]['replaced'] < 3) {
                            ...
                            if ($row['typeID'] == NULL && $realisationTimes[$rtKey]['replaced'] < 2) {
                                $realisationTimes[$rtKey]['days'] = $row['days'];
                                $realisationTimes[$rtKey]['pricePercentage'] = $row['pricePercentage'];
        ...
        $loggedUser = $this->AuthAssistant->getLoggedUser();
     ...
     * @throws Exception
     */
    private function _calcPrice($groupID, $typeID, $formatID, $formatWidth, $formatHeight, $pages, $volume, $attributes, $attributeAmount)
    {
        ...
        foreach ($printTypes as $printType) {
            ...
            foreach ($workspaces as $workspace) {
                ...
                $formatInfo = $this->CalculateStorage->getFormat($formatID);
                $area = $this->getArea($volume, $rows, $rollLength, $setIncrease, $formatInfo['slope']);

                $totalArea = $this->getTotalArea($workspace, $volume, $rollLength, $setIncrease);

                $perimeter = $this->_perimeter($volume);
                $netPerimeter = $this->netPerimeter($volume, $formatInfo['slope']);
                ...
                $sheetsRatio = null;
                $perSheetRatio = null;
                if( $originalUsePerSheet ) {
                    $sheetsRatio = ceil($sheets / $calcAllSheets['originalSheets']);

                    if( $sheetsRatio > 1 ) {
                        $projectSheets = $sheetsRatio;
                        $workspaceInfo['projectSheets'] = $projectSheets;
                        $sheets *= $sheetsRatio;
                        $workspaceInfo['sheets'] = $sheets;
                    }
                ...
                if ($workspace['type'] != 3) {
                    ...
                    $sheetsArea = $this->formatWidth / 1000 * $this->formatHeight / 1000;
                    $sheetsArea *= $volume;
                }
                ...
                foreach ($attributes as $attr) {
                    ...
                    if ($activeOption[$optID]['weightPerMeter'] !== null) {
                        ...
                        $netWidth = $formatInfo['width'] - ($formatInfo['slope'] * 2);
                        $netHeight = $formatInfo['height'] - ($formatInfo['slope'] * 2);
     ...
     * @return array
     */
    private function getArea($volume, $rows, $maxRollLength, $setIncrease, $slope)
    {
        $volume = $this->CalculateAdapter->addSetIncreaseToVolume($volume, $setIncrease, $rows);

        $size = $this->CalculateAdapter->calculateSize($this->formatWidth, $this->formatHeight);
        $sizeNet = $this->CalculateAdapter->calculateSizeNet($this->formatWidth, $this->formatHeight, $slope);

        $size *= $volume;
        $sizeNet *= $volume;

        if ($maxRollLength !== null) {
            $length = $this->CalculateAdapter->getLengthForRoll($size, $this->formatWidth);
     ...
     * @return float|int
     */
    private function getTotalArea($workspace, $volume, $maxRollLength, $setIncrease)
    {
     ...
     * @return int
     */
    private function _perimeter($volume)
    {
        $width = $this->formatWidth;
        $height = $this->formatHeight;
        $width /= 1000;    //z mm na metry
        $height /= 1000;
     ...
     * @return float|int
     */
    private function netPerimeter($volume, $formatSlope)
    {
        $width = $this->formatWidth - ($formatSlope * 2);
        $height = $this->formatHeight - ($formatSlope * 2);
    ...
    private function _attrPrice($attrID, $optID, $printTypeID, $workspace, $pricelistID, $sheets, $projectSheets,
                                ...
                                $totalSheetFolds, $sheetIncrease, $setIncrease, $expense = false)
    {
        ...
        if ($this->formatWidth > $this->formatHeight) {
            $longSide = $this->formatWidth;
            $shortSide = $this->formatHeight;
        } else {
            $longSide = $this->formatHeight;
            $shortSide = $this->formatWidth;
        }
        ...
        $increases = $this->_attrIncreases($controllerID, $sheets, $projectSheets);
            ...
            foreach ($priceTypes as $priceType) {
                ...
                switch ($priceType['function']) {
                    ...
                    case 'allAreasLength':
                        ...
                        $length = (($this->formatWidth * 2) + ($this->formatHeight * 2)) / 1000 * ceil($uzytki);
                    ...
                    case 'lengthForWidth':
                        if ($attributeAmount > 1) {
                            $range = ($this->formatWidth * $attributeAmount) / 1000;
                            $amount = ($this->formatWidth * $attributeAmount) / 1000 * $volume;
                        } else {
                            $range = $this->formatWidth / 1000;
                            $amount = $this->formatWidth / 1000 * $volume;
                        }
                        break;
                    case 'lengthForHeight':
                        if ($attributeAmount > 1) {
                            $range = ($this->formatHeight * $attributeAmount) / 1000;
                            $amount = ($this->formatHeight * $attributeAmount) / 1000 * $volume;
                        } else {
                            $range = $this->formatHeight / 1000;
                            $amount = $this->formatHeight / 1000 * $volume;
                        }
                        break;
                    case 'squareMeterNet':
     ...
     * @return array
     */
    private function _attrIncreases($controllerID, $sheets, $projectSheets)
    {
